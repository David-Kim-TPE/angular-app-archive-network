...
export class UserService {
  ...
  checkWallet() {
    return this.httpClient.get('http://localhost:3000/api/wallet', { withCredentials: true, responseType: 'json' })
      .toPromise()
      .then(results => {
        this.cards = results;
  ...
  signUp(data, callback = null) {
    ...
    const participantUser = {
      $class: `org.example.archive.${role}`,
      'personId': data.id,
      'name': data.firstName,
      'lastName': data.surname
    };

    return this.httpClient.post(`http://localhost:3001/api/${role}`, participantUser)
      .toPromise()
      .then(res => {
        const identity = {
          participant: `org.example.archive.${role}#${data.id}`,
          userID: data.id,
          options: {}
        };
  ...
  getCurrentUser() {
    return this.httpClient.get('http://localhost:3000/api/system/ping', { withCredentials: true })
      .toPromise()
      .then(data => {
  ...
  getUserCard(cardName) {
    return this.httpClient.get('http://localhost:3000/api/wallet/' + cardName + '/export', { withCredentials: true })
      .toPromise()
      .then((result: Blob) => {
  ...
  bindUserCard(cardName) {
    ...
    return this.httpClient.post('http://localhost:3000/api/wallet/' + cardName + '/setDefault', params, { withCredentials: true })
      .toPromise()
      .then(res => {
        console.log('res: ', res);
      })
